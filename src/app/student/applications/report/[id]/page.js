      ...
      {report.questions && report.questions.length > 0 && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 md:p-8 mb-6">
          <h2 className="text-lg font-bold text-slate-800 mb-6 flex items-center gap-2">
            ...
            Question Review ({report.questions.length} Questions)
          </h2>

          <div className="space-y-5">
            {report.questions.map((q, idx) => {
              const optionLabels = ['A', 'B', 'C', 'D'];
              const isUnanswered = q.selectedOptionIndex === -1;
              const statusText = isUnanswered ? 'Not Answered' : q.isCorrect ? 'Correct' : 'Incorrect';
              const statusColor = isUnanswered ? 'bg-slate-100 text-slate-500' : q.isCorrect ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-600';
              const statusIcon = isUnanswered ? '⚪' : q.isCorrect ? '✅' : '❌';
              ...
              return (
                <div key={idx} className="border border-slate-200 rounded-xl p-5 hover:border-slate-300 transition-colors">
                  ...
                  <div className="flex items-start justify-between gap-3 mb-4">
                    ...
                    <span className={`flex-shrink-0 px-3 py-1 rounded-full text-xs font-bold ${statusColor}`}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 ml-11">
                    {(q.options || []).map((opt, oi) => {
                      ...
                      if (isCorrectOpt && isSelected) {
                        optionStyle = 'bg-emerald-50 border-emerald-300 text-emerald-800 ring-2 ring-emerald-200';
                      } else if (isCorrectOpt) {
                        optionStyle = 'bg-emerald-50 border-emerald-300 text-emerald-700';
                      } else if (isSelected) {
                        optionStyle = 'bg-red-50 border-red-300 text-red-700 ring-2 ring-red-200';
                      }

                      return (
                        <div key={oi} className={`flex items-start gap-2 px-3 py-2.5 rounded-lg border text-sm transition-colors ${optionStyle}`}>
                          <span className="font-bold flex-shrink-0">{optionLabels[oi]}.</span>
                          <span className="flex-1">{opt}</span>
                          {isCorrectOpt && (
                            <span className="flex-shrink-0 text-emerald-600 text-xs font-bold">✓ Correct</span>
                          )}
                          {isSelected && !isCorrectOpt && (
                            <span className="flex-shrink-0 text-red-500 text-xs font-bold">✗ Your Answer</span>
                          )}
                          {isSelected && isCorrectOpt && (
                            <span className="flex-shrink-0 text-emerald-600 text-xs font-bold">✓ Your Answer</span>
                          )}
                        </div>
                      );
                    })}
                  ...
                  {/* Explanation */}
                  {q.explanation && (
                    <div className="ml-11 mt-3 bg-blue-50 border border-blue-100 rounded-lg p-3">
                  ...
                  <div className="ml-11 mt-3 flex flex-wrap gap-2">
                    ...
                    {q.difficulty && (
                      <span className={`px-2.5 py-1 rounded-md text-xs font-semibold ${
                        ...
                        {q.difficulty === 'Hard' ? '🔴' : q.difficulty === 'Medium' ? '🟡' : '🟢'} {q.difficulty}
                      </span>
                    )}
                  </div>
                </div>
