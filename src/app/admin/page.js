  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
        {kpis.map((kpi, idx) => (
          <div key={idx} className={`bg-white p-6 rounded-2xl border transition-all duration-300 transform flex items-center gap-4 shadow-[0_4px_20px_rgba(0,0,0,0.01)] group ${kpi.borderHover}`}>
            <div className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-transform duration-300 group-hover:scale-110 shadow-sm ${kpi.bg} ${kpi.color}`}>
              {kpi.icon}
            </div>
            <div>
